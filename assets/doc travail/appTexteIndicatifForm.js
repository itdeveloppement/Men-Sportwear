...
// IMPORTS
import { openModal } from '../js/fonctionsAffichage.js';
import { changeTypo } from '../js/fonctionsAffichage.js';
...
// TEXTE INDICATIF EN JS

// tableau du nom des champs avec le texte indicatif corespondant
const tabTextIndicatifForm = [
    {nom: "nom", text:"Votre nom"},
    {nom: "prenom", text:"Votre prenom"},
    {nom: "email", text:"exemple@exemple.exemple"},
    {nom: "reference-produit", text:"XXX-000000"},
    {nom: "message", text:"Votre message"},
];
...
*/
function testPrenom(){
    let champs = document.getElementById("prenom");
    let textIndicatifPrenom = textIndChamps(champs)
    // si le champs est vide
    if(beEmpty (champs.value, textIndicatifPrenom)){
        // affiche le message d'erreur et bordure
        afficheErreur("prenom", "Ce champs doit etre complété");
        return false
     // si le texte dépasse x caracteres // 
    }else if(maxLength(champs.value, maxLengthPrenom)){
        afficheErreur("prenom", `Ce champ ne peux pas depasser ${maxLengthPrenom} caracteres`);
        return false
    // si caractere non alphanumerique
    } else if (onlyAlphanumeric(champs.value )=== false) {
...
function testNom(){
    ...
    let textIndicatifNom = textIndChamps(champs)
    // si le champs est vide
    if(beEmpty (champs.value, textIndicatifNom)){
        // affiche le message d'erreur et bordure
        afficheErreur("nom", "Ce champs doit etre complété");
        return false
     // si le texte dépasse x caracteres // 
    }else if(maxLength(champs.value, maxLengthNom)){
        afficheErreur("nom", `Ce champ ne peux pas depasser ${maxLengthNom} caracteres`);
        return false
    // si caractere non alphanumerique
    } else if (onlyAlphanumeric(champs.value )=== false) {
...
*/
function testEmail(){
    let champs = document.getElementById("email");
    let textIndicatifEmail = textIndChamps(champs)
    // si le champs est vide
    if(beEmpty (champs.value, textIndicatifEmail)){
        // affiche le message d'erreur et bordure
        afficheErreur("email", "Ce champs doit etre complété");
        return false
     // si le texte dépasse x caracteres // 
    }else if(maxLength(champs.value, maxLengthEmail)){
        afficheErreur("email", `Ce champ ne peux pas depasser ${maxLengthEmail} caracteres`);
        return false
    // si injection de code
    }else if(hasCode(champs.value)){ 
    ...
    } else if (formatEmail (champs.value)=== false) {
...
function testRefProduit() {
    ...
    let textIndicatifRefProduit = textIndChamps(champs)
    // si le champs est vide
    if(beEmpty (champs.value, textIndicatifRefProduit)){
        // affiche le message d'erreur et bordure
...
/** CHAMPS MESSAGE : verifie les données di champs
*       si la valeur du champs est vide
...
function testMessage(){
    let champs = document.getElementById("message");
    let textIndicatifMessage = textIndChamps(champs)
    if (beEmpty(message.value, textIndicatifMessage)){
...
// FORMULAIRE AUTRE FONCTION
/** recuperation dans un tableau de la valeur du texte indicatif associée au champs
 * @param {*} champs le champ ciblé
 * @returns {string} la valeur de la chaine de caractere texte indicatif du champ
 */
function textIndChamps(champs) {
    let textIndChamps = "";
    tabTextIndicatifForm.forEach(element => {
        if (element.nom === champs.name) {
            textIndChamps = element.text;
        }
    });
    return textIndChamps;
}
// TEXTE INDICATIF  DES CHAMPS

// au chargement remplissage des champs
tabTextIndicatifForm.forEach(element => {
    let champs = document.getElementById(element.nom);
    champs.value = element.text;
    champs.classList.add ("texte-indicatif");
});
// à l'utilisation du formulaire insertion ou supression du texte indicatif quand l'utilisateur entre ou sort du champ
tabTextIndicatifForm.forEach(element => {
    //ciblage
    let champs = document.getElementById(element.nom);

    // pose ecouteur à l'entrée dans le champ
    champs.addEventListener("focus", ()=> { 
    // si valeur du champ === texte indicatif
    if (champs.value === element.text) {
        // vide le champ
        champs.value = "";
        // enleve la classe du format du champ indicatif
        champs.classList.remove ("texte-indicatif");
    }
    });
    // pose ecouteur à la sortie du champ
    champs.addEventListener("blur", ()=> { 
     // si valeur du champ === vide
    if (champs.value === "") {
        // rempli le champ
        champs.value = element.text;
        // ajoute la classe du format du champ indicatif
        champs.classList.add ("texte-indicatif");
    }
    });
});
